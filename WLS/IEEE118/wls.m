...
truestate = xlsread('truetrainstate.xlsx');
truestate = truestate(:,2:end)';


% nummsr = 359;  %%total:nummsr+1
% 
% while true
%     ii=100;
%     while ii
...
nummsr = 93;  %%total:nummsr+1
pq = linspace(1,237,237);
while true
    ii=100;
    while ii
        index = randperm(358);
%         index(find(index==1))=[];
        ...
        for set=1:c
            ...
            del = 30*pi/180*ones(nbus,1); % Initialize the bus angles..
            E = [del(1:68); del(70:end); V];   % #69-the reference bus
            ...
            while(tol > 1e-5 && iter <= 100)
                ...
                if iter == 100
                    NG = NG +1;
                    disp('ooooooooooooooooooooooooooooooooooooooooo')
                end
                h1 = V(fbus(vi),1);
                h2 = zeros(npi,1);
                h3 = zeros(nqi,1);
                ...
                for i = 1:nqf
                    m = fbus(qf(i));
                    n = tbus(qf(i));
                    h5(i) = -V(m)^2*(-B(m,n)) - V(m)*V(n)*(-G(m,n)*sin(del(m)-del(n)) + B(m,n)*cos(del(m)-del(n))); %
                end
                ...
                for k = 1:nvi
                    for n = 1:nbus
                        if n == k %%This is only for the consective volatage magnitude measuremnts
                            H12(k,n) = 1;
                        end
                    end
                end
                ...
                E = E + dE;
                del(1:68) = E(1:68);
                del(70:end) = E(69:nbus-1);
                V = E(nbus:end);
                iter = iter + 1;
                tol = max(abs(dE));
            end
            
            maee = mean(abs(E-truestate(:,set)));
            
            if maee > 0.1
                badwls = horzcat(badwls,set);
            end
    ...
    if done == 1
        ...
        disp(max(TT));
        xlswrite('measurements.xlsx',max(TT),'MAX');
